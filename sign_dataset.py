...
def read_pose_file(filepath, include_confidence=False):
    ...
    try:
        ...
        conf = [
            v
            for i, v in enumerate(body_pose)
            if i % 3 == 2 and i // 3 not in body_pose_exclude
        ]
        ...
        conf = torch.FloatTensor(conf).unsqueeze(1)

        xy = torch.stack([x, y]).transpose_(0, 1)

        if include_confidence:
            ft = torch.cat([xy, conf, x_diff, y_diff, orient], dim=1)
...
class Sign_Dataset(Dataset):
    def __init__(
        ...
        self.data = []
        self.label_encoder, self.onehot_encoder = LabelEncoder(), OneHotEncoder(
            categories="auto"
        )
    ...
    def _load_poses(
        ...
        for i in frames_to_sample:
            ...
            pose = read_pose_file(pose_path, include_confidence)
