...
def run(split_file, pose_data_root, configs, save_model_to=None):
    ...
    for epoch in range(int(epochs)):
        ...
        train_losses, train_scores, train_gts, train_preds = train(
            log_interval, None, train_data_loader, None, epoch
        )
        print("start testing.")
        val_loss, val_score, val_gts, val_preds, incorrect_samples = validation(
            None, val_data_loader, epoch, save_to=save_model_to
        )
        # print('start testing.')
...
if __name__ == "__main__":
    ...
    pose_data_root = "./data/pose_per_individual_videos"
    config_file = "./configs/{}.ini".format(subset)
    configs = Config(config_file)

    logging.basicConfig(
        filename="output/{}.log".format(os.path.basename(config_file)[:-4]),
        level=logging.DEBUG,
        filemode="w+",
    )
