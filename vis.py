...
def main(video_idx):
    ...
    ax.set_aspect("equal")
    ax.invert_yaxis()
    ax.set_title(f"Pose Animation from video: {video_id[0]}. Gloss: {y[0]}")
    ...
    def on_key_press(event):
        nonlocal paused, current_frame, anim
        if event.key == " ":
            if paused:
                anim.resume()
            else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Display pose animation for a specific video."
    )
    parser.add_argument("video_idx", type=int, help="Index of the video to display")
